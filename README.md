...
![Intro Workflow Diagram](./assets/images/new_doc_workflow.png)
...
![Continuation Workflow Diagram](./assets/images/update_doc_workflow.png)
...
├── test_data/                       # 파이프라인 E2E 검증용 모의 데이터
└── src/
    ...
    └── nodes/                       # 에이전트 노드 구현부
        ├── main_node.py             # Supervisor / QA Critic / HITL / Final Publish Agent
        └── sub_graph_nodes/
            ├── new_doc_graph_node.py        # 신규 문서 전용 파이프라인 노드
            ├── update_doc_graph_node.py     # 업데이트 문서 전용 파이프라인 노드
            └── common_node.py               # 공통 노드 (다이어그램 분석, 이미지 배치)
```