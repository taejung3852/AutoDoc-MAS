...
# ==========================================
# 🖥️ 메인 화면 및 헤더 (설정 버튼 포함)
# ==========================================
col_header1, col_header2 = st.columns([8, 2])
with col_header1:
    st.header(st.session_state.get('selected_system', '새로운 시스템') if st.session_state.get('selected_system') != "NEW" else "✨ 새로운 기술 문서 파이프라인 생성")
with col_header2:
...
doc_type = st.selectbox("산출물 유형", ["기능 명세서 (Feature Spec)", "릴리즈 노트 (Release Note)", "API 연동 문서 (API Doc)"])

col_input1, col_input2 = st.columns(2)
with col_input1:
    technical_source = st.text_area("파편화된 원시 기술 데이터 입력 (회의록, 로그, 슬랙 등)", height=250)
with col_input2:
    uploaded_diagrams = st.file_uploader("다이어그램 / 아키텍처 이미지 (선택)", type=['png', 'jpg'], accept_multiple_files=True)

if st.button("🚀 AutoDoc-MAS 파이프라인 가동", use_container_width=True, type="primary"):
    if not system_name or not technical_source:
        st.error("시스템 식별자와 기술 데이터는 필수입니다.")
        st.stop()
...
if 'thread_id' in st.session_state:
    ...
    if is_paused:
        ...
        with view_col:
            ...
            st.markdown(edited_text) # 오른쪽 에디터의 내용을 실시간 렌더링
        
        st.markdown("---")
        c1, c2 = st.columns(2)
        # help 파라미터를 사용하여 마우스 호버 시 나타나는 툴팁(ⓘ) 추가
        c1, c2 = st.columns(2)
